        ...
        {showForm && (
          <div className="card mb-6 border-brand-green/20">
            ...
            <form onSubmit={handleSaveCam} className="space-y-4">
              ...
              <div>
                <label className="label">Stream URL * <span className="text-xs text-brand-muted">(YouTube Live, HLS .m3u8, or direct stream link)</span></label>
            ...
            {cams.map(cam => (
              <div key={cam.id} className="card hover:border-brand-borderHover transition group">
                ...
                <div className="flex gap-2 pt-3 border-t border-brand-border">
                  ...
                  <button onClick={() => navigator.clipboard?.writeText(`https://hiddencameras.tv/cam/${cam.share_token}`)} className="btn-ghost text-xs py-1.5 px-3">
                    Copy Link
                  </button>
